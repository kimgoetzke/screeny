...
import canvasSource from "./Canvas.svelte?raw";
...
describe("Canvas", () => {
  ...
  it("hides the empty-state copy while a file is being dragged over the canvas", () => {
    const { body } = render(Canvas, {
      props: {
  ...
  it("shows a background grid even when no GIF is loaded", () => {
    ...
    expect(body).toContain('data-testid="canvas-grid-fade"');
    expect(body).toContain('data-testid="canvas-grid-stage"');
    expect(body).toContain('data-testid="canvas-grid"');
    expect(body).toContain('data-testid="canvas-empty"');
  });

  describe("wheel zoom", () => {
    it("handleWheel does not require ctrlKey to zoom", () => {
      expect(canvasSource).not.toMatch(/if\s*\(\s*!event\.ctrlKey\s*\)\s*return/);
    });
  });

  describe("zoom/pan transform", () => {
    it("renders a shared canvas stage with the default transform", () => {
      const { body } = render(Canvas);

      expect(body).toContain('data-testid="canvas-stage"');
      expect(body).toContain('data-testid="canvas-grid-stage"');
      expect(body).toContain("transform: scale(1) translate(0px, 0px)");
    });

    it("applies the provided scale and pan to the shared canvas stage", () => {
      frameStore.setFrames([makeFrame("a")]);
      const { body } = render(Canvas, { props: { scale: 2, panX: 50, panY: -30 } });
  ...
  describe("grid centring and fade", () => {
    it("centres the grid pattern and fades it out through a dedicated shell", () => {
      expect(canvasSource).toContain("background-position: center center;");
      expect(canvasSource).toContain("border-radius: 50%;");
      expect(canvasSource).toContain(".canvas-grid-fade");
      expect(canvasSource).toContain("transparent 100%");
    });
  });
  ...
  describe("redraw guard during streaming load", () => {
    ...
    it("reads the frame data under untrack so the effect does not depend on the frames array", () => {
      expect(canvasSource).toContain("untrack");
      expect(canvasSource).toMatch(
  ...
  describe("inspector-aware centering", () => {
    it("sets the fade mask centre CSS variable from panX", () => {
      frameStore.setFrames([makeFrame("a")]);
      const { body } = render(Canvas, { props: { panX: -132.5 } });
